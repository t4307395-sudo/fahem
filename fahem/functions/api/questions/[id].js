...
export async function onRequestPut({ request, env, params }) {
  ...
  const result = await env.DB.prepare('UPDATE questions SET subject=?,unit=?,lesson=?,school_year=?,educational_stage=?,type=?,prompt=?,options_json=?,correct_answer=?,explanation=?,difficulty=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(subject, unit, lesson, limitText(x.school_year, 80) || null, limitText(x.educational_stage || x.stage, 120) || null, x.type, prompt, JSON.stringify(options), limitText(x.correct_answer, 1000) || null, limitText(x.explanation, 20000) || null, difficulty, id).run();
