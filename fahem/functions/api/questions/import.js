...
export async function onRequestPost({ request, env }) {
  ...
  const statements = unique.map(q => env.DB.prepare('INSERT INTO questions(subject,unit,lesson,school_year,educational_stage,type,prompt,options_json,correct_answer,explanation,difficulty) VALUES(?,?,?,?,?,?,?,?,?,?,?)').bind(q.subject, q.unit, q.chapter, q.school_year || null, q.educational_stage || null, q.type, q.prompt, JSON.stringify(q.options), q.correct_answer || null, q.explanation || null, q.difficulty));
