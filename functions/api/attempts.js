...
export async function onRequestPost({ request, env }) {
  ...
  const ids = new Set(x.question_ids.map(Number).filter(Number.isInteger));
  ...
  let score = 0, total = 0;
  const attempt = await env.DB.prepare('INSERT INTO attempts(student_id,mode,completed_at) VALUES(?,?,CURRENT_TIMESTAMP) RETURNING id').bind(sid, mode).first();
  for (const q of questions) { const answer = limitText(x.answers?.[q.id], 20000) || null; const correct = q.type === 'essay' ? null : answer === q.correct_answer; if (q.type !== 'essay') { total++; if (correct) score++; } await env.DB.prepare('INSERT INTO attempt_answers(attempt_id,question_id,answer_text,automatic_correct) VALUES(?,?,?,?)').bind(attempt.id,q.id,answer,correct === null ? null : (correct ? 1 : 0)).run(); }
