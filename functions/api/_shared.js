...
export async function studentId(user, env) { const email=String(user?.email||'').trim().toLowerCase(); return email ? (await env.DB.prepare('SELECT id FROM students WHERE lower(email)=?').bind(email).first())?.id : null; }
