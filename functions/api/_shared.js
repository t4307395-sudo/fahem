...
export async function loginRateAllowed(request, env, email) { const ip = request.headers.get('CF-Connecting-IP') || request.headers.get('x-forwarded-for')?.split(',')[0]?.trim() || 'unknown'; const key = `${ip}:${email}`; const now = Math.floor(Date.now() / 60000); const row = await env.DB.prepare('SELECT window_start,attempts FROM login_rate_limits WHERE key=?').bind(key).first(); if (row?.window_start === now && row.attempts >= 10) return false; await env.DB.prepare('INSERT INTO login_rate_limits(key,window_start,attempts) VALUES(?,?,1) ON CONFLICT(key) DO UPDATE SET window_start=excluded.window_start,attempts=CASE WHEN login_rate_limits.window_start=excluded.window_start THEN login_rate_limits.attempts+1 ELSE 1 END').bind(key, now).run(); return true; }
