...
export async function onRequestGet({ request, env }) {
  ...
  const teacher = ['admin','teacher'].includes(user?.role);
  ...
  if (!teacher && user?.role === 'student') {
    ...
    if (!profile?.educational_stage || !profile?.school_year) return json({ questions: [], needsCurriculum: true });
    const rows = await env.DB.prepare(`SELECT ${columns},q.lesson AS chapter FROM questions q WHERE q.educational_stage=? AND q.school_year=? AND q.is_published=1 ORDER BY q.subject,q.unit,q.lesson,q.id DESC`).bind(profile.educational_stage, profile.school_year).all();
...
export async function onRequestPost({ request, env }) {
  ...
  const q = await env.DB.prepare('INSERT INTO questions(subject,unit,lesson,school_year,educational_stage,type,prompt,options_json,correct_answer,explanation,difficulty,is_published) VALUES(?,?,?,?,?,?,?,?,?,?,?,?) RETURNING id').bind(subject, unit, lesson, limitText(x.school_year, 80) || null, limitText(x.educational_stage || x.stage, 120) || null, x.type, prompt, JSON.stringify(options), limitText(x.correct_answer, 1000) || null, limitText(x.explanation, 20000) || null, difficulty, x.is_published === false ? 0 : 1).first();
  return json({ id: q.id });
}
