...
export async function onRequestPost({ request, env }) {
  ...
  const q = await env.DB.prepare('INSERT INTO questions(subject_id,chapter_id,type,prompt,options_json,correct_answer,explanation,difficulty) VALUES(?,?,?,?,?,?,?,?) RETURNING id').bind(s.id,c.id,x.type,x.prompt.trim(),JSON.stringify(x.options || []),x.correct_answer || null,x.explanation || null,difficulty).first();
  return json({ id: q.id });
}
