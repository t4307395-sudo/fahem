...
export async function onRequestGet({ request, env }) {
  ...
  if (!user) {
    const rows = await env.DB.prepare(`SELECT q.id,q.subject,q.unit,q.lesson,q.school_year,q.educational_stage,q.type,q.prompt,q.options_json,q.correct_answer,q.explanation,q.difficulty,q.created_at,q.updated_at,q.lesson AS chapter FROM questions q WHERE q.is_published=1 ORDER BY q.school_year,q.subject,CASE WHEN q.unit LIKE '%الأولى%' THEN 1 WHEN q.unit LIKE '%الثانية%' THEN 2 WHEN q.unit LIKE '%الثالثة%' THEN 3 WHEN q.unit LIKE '%الرابعة%' THEN 4 ELSE 99 END,CASE WHEN q.lesson LIKE '%الأول%' THEN 1 WHEN q.lesson LIKE '%الثاني%' THEN 2 WHEN q.lesson LIKE '%الثالث%' THEN 3 WHEN q.lesson LIKE '%الرابع%' THEN 4 WHEN q.lesson LIKE '%الخامس%' THEN 5 WHEN q.lesson LIKE '%السادس%' THEN 6 ELSE 99 END,q.id`).all();
  ...
  if (!teacher && user?.role === 'student') {
    ...
    if (!profile?.school_year) {
      ...
      return json({questions:(rows.results||[]).map(q=>({...q,options:optionsJson(q.options_json)})), curriculumChoice:true});
    }
...
export async function onRequestPost({ request, env }) {
  ...
  const options=Array.isArray(x.options)?x.options.map(v=>limitText(v,1000)).filter(Boolean).slice(0,10):[]; const difficulty=['easy','medium','hard'].includes(x.difficulty)?x.difficulty:'medium'; const correct_answer=limitText(x.correct_answer,1000)||null;
  if(x.type!=='essay'&&(!correct_answer||!options.includes(correct_answer)))return json({error:'الإجابة الصحيحة يجب أن تطابق أحد الاختيارات حرفيًا'},400);
  const q=await env.DB.prepare('INSERT INTO questions(subject,lesson,school_year,educational_stage,type,prompt,options_json,correct_answer,explanation,difficulty,is_published) VALUES(?,?,?,?,?,?,?,?,?,?,?) RETURNING id').bind(subject,lesson,limitText(x.school_year,100)||null,limitText(x.educational_stage,80)||null,x.type,prompt,JSON.stringify(options),correct_answer,limitText(x.explanation,20000)||null,difficulty,x.is_published===false?0:1).first(); return json({id:q.id});
}
