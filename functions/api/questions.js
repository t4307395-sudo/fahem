...
export async function onRequestGet({ request, env }) {
  ...
  const columns = teacher ? 'q.*' : 'q.id,q.subject,q.lesson,q.school_year,q.educational_stage,q.type,q.prompt,q.options_json,q.explanation,q.difficulty,q.created_at,q.updated_at';
  if (!teacher && user?.role === 'student') {
    ...
    const rows = await env.DB.prepare(`SELECT ${columns},q.lesson AS chapter FROM questions q WHERE q.school_year=? AND (q.educational_stage=? OR q.educational_stage IS NULL OR trim(q.educational_stage)='') AND q.is_published=1 ORDER BY q.subject,q.lesson,q.id DESC`).bind(profile.school_year, profile.educational_stage || '').all();
    return json({questions:(rows.results||[]).map(q=>({...q,options:optionsJson(q.options_json)}))});
  }
  const rows = await env.DB.prepare(`SELECT ${columns},q.lesson AS chapter FROM questions q ORDER BY q.subject,q.lesson,q.id DESC`).all();
  return json({questions:(rows.results||[]).map(q=>({...q,options:optionsJson(q.options_json)}))});
}
