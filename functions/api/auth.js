...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!user) {
      ...
      if(user.role === 'student' && (name || educational_stage || school_year)) { if(!name || !educational_stage || !school_year) return json({ error: 'أكمل الاسم والمرحلة والسنة الدراسية' }, 400); await env.DB.prepare('UPDATE users SET last_seen_at=CURRENT_TIMESTAMP,name=?,educational_stage=?,school_year=? WHERE id=?').bind(name, educational_stage, school_year, user.id).run(); user.name = name; user.educational_stage = educational_stage; user.school_year = school_year; } else if(user.role === 'student') { await env.DB.prepare('UPDATE users SET last_seen_at=CURRENT_TIMESTAMP WHERE id=?').bind(user.id).run(); }
    }
