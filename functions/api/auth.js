import { json, body, me, sha, session, cookie, originGuard, limitText } from './_shared.js';
...
export async function onRequestPost({ request, env }) {
  ...
  if (!student) student = await env.DB.prepare('INSERT INTO students(name,email,password_hash) VALUES(?,?,?) RETURNING *').bind(name, email, await sha(password)).first();
  else if (!student.password_hash || student.password_hash !== await sha(password)) return json({ error: 'بيانات الدخول غير صحيحة' }, 401);
