...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    stage = 'body'; const x = await body(request);
    const email = String(x.email || '').trim().toLowerCase(); const password = String(x.password || ''); const name = limitText(x.name, 120);
    ...
    stage = 'response'; return json({ user: { id: user.id, email: user.email, name: user.name, role } }, 200, { 'Set-Cookie': cookie(token) });
  } catch (error) {
